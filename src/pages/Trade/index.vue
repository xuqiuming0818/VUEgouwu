<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="(Address,index) in AddressList" :key="Address.id" @click="seletAddress(Address,AddressList)">
...
import {mapState} from 'vuex'
  export default {
    ...
    // 点击默认地址
    methods: {
        seletAddress(Address,AddressList){
      ...
      async submitOrder(){
        // 需要带参数
        let data = {
          consignee:this.userAddress.consignee,//名字
          consigneeTel:this.userAddress.phoneNum,//电话
          deliveryAddress:this.userAddress.fullAddress,//地址
        ...
        if (result.data.code == 200) {
          console.log(result);
          this.ordId = result.data.data
          // 跳转路由 + 路由传参
          this.$router.push('/pay?orderId='+this.ordId)
        }else{//提交订单失败
