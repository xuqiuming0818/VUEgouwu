...
export default {
  ...
  methods: {
    //添加到购物车操作
    async addShopCar() {
      //跳转路由前处理好数据，并且发送请求  把购买产品的信息，通知服务器  服务器存储
      //说白了：调用了仓库的addOrUpdateShopCart，这个方法加上了async，返回的一定是一个Promise
      // 返回成功，即可使用
      try {
        ...
        this.$router.push({
          name: "addcartsuccess",
          query: { skuNum: this.skuNum }, //产品数据比较复杂，比如【skuInfo】，通过会话存储（不持久化，会话结束数据）
        });
      } catch (error) {
        alert(error);
      }
